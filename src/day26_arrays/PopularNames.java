...
public class PopularNames {
	public static void main(String[] args) {
		...
		for (int i = 0; i < names.length; i++) {
			if (names[i].length() < 5) {
				namesUpto4 +=names[i].toUpperCase() + " ";
		} else if (names[i].length() < 7 && names[i].length() > 5) {
			names5to6 +=names[i].toUpperCase() + " ";
	} else if (names[i].length() > 6 ) {
		names7plus +=names[i].toUpperCase() + " ";
}
	}
		System.out.println("\nNames with 4 characters or less: "+ namesUpto4);
		System.out.println("\nNames with between 5 and 6 charcaters: "+ names5to6);
		System.out.println("\nNames with 7 or more characters: "+ names7plus);
